...
const AI_PROVIDERS = [
  { id: "groq", name: "Groq (FREE)", model: "llama-3.3-70b-versatile", url: "https://api.groq.com/openai/v1/chat/completions", free: true, hint: "console.groq.com" },
  { id: "gemini", name: "Google Gemini (FREE)", model: "gemini-2.0-flash", url: "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent", free: true, hint: "aistudio.google.com/apikey" },
  { id: "anthropic", name: "Anthropic Claude", model: "claude-sonnet-4-20250514", url: "https://api.anthropic.com/v1/messages", free: false, hint: "console.anthropic.com" },
  { id: "openai", name: "OpenAI GPT-4o", model: "gpt-4o", url: "https://api.openai.com/v1/chat/completions", free: false, hint: "platform.openai.com" },
  { id: "custom", name: "Custom / Proxy", model: "", url: "", free: true, hint: "Your own endpoint" },
];

function SettingsPanel({ config, onSave }) {
  const [provider, setProvider] = useState(config.provider || "groq");
  const [apiKey, setApiKey] = useState(config.apiKey || "");
  const [customUrl, setCustomUrl] = useState(config.customUrl || "http://localhost:3001");
  const [customModel, setCustomModel] = useState(config.customModel || "");
  const [showKey, setShowKey] = useState(false);
  const [testing, setTesting] = useState(false);
  const [testResult, setTestResult] = useState(null);
  const sel = AI_PROVIDERS.find(p => p.id === provider);

  const testConn = async () => {
    setTesting(true); setTestResult(null);
    try {
      const url = provider === "custom" ? customUrl : sel.url;
      const model = provider === "custom" ? customModel : sel.model;
      let res;
      if (provider === "gemini") {
        res = await fetch(url + "?key=" + apiKey, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ contents: [{ parts: [{ text: "hi" }] }] }) });
      } else if (provider === "anthropic") {
        res = await fetch(url, { method: "POST", headers: { "Content-Type": "application/json", "x-api-key": apiKey, "anthropic-version": "2023-06-01", "anthropic-dangerous-direct-browser-access": "true" }, body: JSON.stringify({ model, max_tokens: 10, messages: [{ role: "user", content: "hi" }] }) });
      } else {
        res = await fetch(url, { method: "POST", headers: { "Content-Type": "application/json", "Authorization": "Bearer " + apiKey }, body: JSON.stringify({ model, max_tokens: 10, messages: [{ role: "user", content: "hi" }] }) });
      }
      const d = await res.json();
      setTestResult(res.ok ? { ok: true, msg: "Connected!" } : { ok: false, msg: d?.error?.message || "Error " + res.status });
  ...
  return (
    <Card>
      <SectionHeader icon={Icon.settings} label="AI Setup" color={C.accent} />
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        <div style={{ color: C.muted, fontSize: 11, textTransform: "uppercase", letterSpacing: "0.08em" }}>Choose Provider</div>
        {AI_PROVIDERS.map(p => (
          <div key={p.id} onClick={() => setProvider(p.id)} style={{ padding: "10px 12px", borderRadius: 10, border: `1px solid ${provider === p.id ? C.accent : C.border}`, background: provider === p.id ? `${C.accent}10` : C.surface, cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div>
              <div style={{ color: provider === p.id ? C.accent : C.text, fontSize: 13, fontWeight: 600 }}>{p.name}</div>
              <div style={{ color: C.muted, fontSize: 10, marginTop: 2 }}>{p.hint}</div>
            </div>
            {p.free && <span style={{ color: C.green, fontSize: 10, background: `${C.green}15`, padding: "2px 6px", borderRadius: 6 }}>FREE</span>}
          </div>
        ))}
        {provider !== "custom" && (
          <div>
            <div style={{ color: C.muted, fontSize: 11, marginBottom: 6 }}>API KEY</div>
            <div style={{ display: "flex", gap: 6 }}>
              <input type={showKey ? "text" : "password"} value={apiKey} onChange={e => setApiKey(e.target.value)} placeholder="Paste key here..." style={{ flex: 1, padding: "10px 12px", borderRadius: 10, border: `1px solid ${C.border}`, background: C.surface, color: C.text, fontSize: 12, outline: "none", fontFamily: "monospace" }} />
              <button onClick={() => setShowKey(!showKey)} style={{ padding: "0 12px", borderRadius: 10, border: `1px solid ${C.border}`, background: C.surface, color: C.muted, cursor: "pointer", fontSize: 11 }}>{showKey ? "Hide" : "Show"}</button>
            </div>
          </div>
        )}
        {provider === "custom" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            <input value={customUrl} onChange={e => setCustomUrl(e.target.value)} placeholder="http://localhost:3001" style={{ padding: "10px 12px", borderRadius: 10, border: `1px solid ${C.border}`, background: C.surface, color: C.text, fontSize: 12, outline: "none", fontFamily: "monospace" }} />
            <input value={customModel} onChange={e => setCustomModel(e.target.value)} placeholder="Model name (optional)" style={{ padding: "10px 12px", borderRadius: 10, border: `1px solid ${C.border}`, background: C.surface, color: C.text, fontSize: 12, outline: "none", fontFamily: "monospace" }} />
          </div>
        )}
...
If the user asks for both (e.g. "turn off lights AND add a water tank panel"), do the system control AND code update.`;
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const _prov = aiConfig.provider || "groq";
      const _key = aiConfig.apiKey || "";
      const _provMap = {
        groq: { url: "https://api.groq.com/openai/v1/chat/completions", model: "llama-3.3-70b-versatile" },
        gemini: { url: "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=" + _key, model: "gemini-2.0-flash" },
        anthropic: { url: "https://api.anthropic.com/v1/messages", model: "claude-sonnet-4-20250514" },
        openai: { url: "https://api.openai.com/v1/chat/completions", model: "gpt-4o" },
        custom: { url: aiConfig.customUrl || "http://localhost:3001", model: aiConfig.customModel || "" },
      };
      const _p = _provMap[_prov] || _provMap.groq;
      const _msgs = [...history, { role: "user", content: userMsg }];
      let _fetchBody, _fetchHeaders;
      if (_prov === "gemini") {
        _fetchHeaders = { "Content-Type": "application/json" };
        _fetchBody = JSON.stringify({ contents: _msgs.map(m => ({ role: m.role === "assistant" ? "model" : "user", parts: [{ text: m.content }] })), systemInstruction: { parts: [{ text: buildSystemPrompt() }] } });
      } else if (_prov === "anthropic") {
        _fetchHeaders = { "Content-Type": "application/json", "x-api-key": _key, "anthropic-version": "2023-06-01", "anthropic-dangerous-direct-browser-access": "true" };
        _fetchBody = JSON.stringify({ model: _p.model, max_tokens: 8000, system: buildSystemPrompt(), messages: _msgs });
      } else {
        _fetchHeaders = { "Content-Type": "application/json", "Authorization": "Bearer " + _key };
        _fetchBody = JSON.stringify({ model: _p.model, max_tokens: 8000, messages: [{ role: "system", content: buildSystemPrompt() }, ..._msgs] });
      }
      const res = await fetch(_p.url, {
        method: "POST",
...
export default function BusAIControl() {
  ...
  const [aiConfig, setAiConfig] = useState(() => { try { return JSON.parse(localStorage.getItem("busOS_ai") || "{}"); } catch { return {}; } });
  const saveAiConfig = cfg => { localStorage.setItem("busOS_ai", JSON.stringify(cfg)); setAiConfig(cfg); };
